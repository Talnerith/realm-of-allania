        ...
        <div className="flex flex-col md:flex-row gap-4 md:gap-6 group relative">
            ...
            <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-all z-10">
                {isOwner && !editingPostId && (
                    <button onClick={() => onEditStart(post)} className="text-slate-500 hover:text-amber-500 bg-slate-900/50 rounded p-1">
                        <Edit3 className="w-4 h-4" />
                    </button>
                )}
                {isAdminOrMod && !editingPostId && (
                    <button onClick={() => onDelete(post.id)} className="text-red-900/50 hover:text-red-500 bg-slate-900/50 rounded p-1">
            ...
            <div className="hidden md:flex flex-col items-center gap-2 w-24 shrink-0">
                <div
                    ...
                    className="w-20 h-20 bg-slate-800 rounded-lg border-2 border-slate-700 overflow-hidden shadow-lg relative bg-cover bg-center cursor-pointer hover:border-amber-500 transition-colors"
                >
                    <img
                        src={post.characterImageUrl || ''}
                        className="w-full h-full object-cover"
