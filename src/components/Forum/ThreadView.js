...
import {
    ...
    Edit3, Loader, Trash2, Shield, Check, User, X, Gavel, ShieldAlert, MessageCircle, Lock, Unlock, BookLock
} from 'lucide-react';
...
function ThreadView({ thread, setView, region, onOpenCodex, onNavigateToRegion, onMessageUser, onRequireAuth, onWikiLink }) {
    ...
    const canLockThread = isAdminOrMod;
    ...
    const handleEditPostStart = useCallback((post) => {
        // Check if thread is locked and user is not a mod
        if (isThreadLocked && !isAdminOrMod) {
            alert("This thread is locked as a Sacred Text. Only moderators can edit posts.");
            return;
        }
    ...
    const handleToggleLock = async () => {
        ...
        if (!window.confirm(`Are you sure you want to ${action} this thread as a Sacred Text? ${newLockState ? 'Regular users will not be able to edit posts in this thread.' : 'Regular users will be able to edit their posts again.'}`)) return;
        try {
