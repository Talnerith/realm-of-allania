...
import {
  ...
  Users, ImageIcon, Loader, Edit3, Save, X, BookLock
} from 'lucide-react';
import ImageUploader from '@/components/ImageUploader';
import MarkdownEditor from '@/components/MarkdownEditor';

function RegionView({ region, setView, setActiveThread }) {
    ...
    try {
      ...
      if (createCodexEntry) {
        await addDoc(collection(db, 'artifacts', APP_ID, 'public', 'data', 'codex_pages'), {
          ...
          updatedAt: serverTimestamp(),
          updatedBy: char.name,
          creatorId: user.uid, // Explicitly set creator
  ...
  const handleCancelCreate = async () => {
    ...
    for (const url of sessionUploads) {
      try {
        ...
        // Track permission errors specifically for user feedback
        if (e.code === 'storage/unauthorized') {
          cleanupErrors.push(url);
        }
      }
    }
    
    // Show non-blocking warning if cleanup failed (non-critical)
  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar bg-linear-to-b from-slate-950 to-slate-900 pb-48">
      ...
      <div className="max-w-4xl mx-auto w-full p-4 md:p-8">
        {/* Cleanup Error Warning */}
        {cleanupError && (
          <div className="mb-4 p-3 bg-amber-900/30 border border-amber-700/50 rounded-lg flex items-center gap-2 text-amber-300 text-sm animate-in fade-in">
        ...
        <div className="space-y-3">
          {threads.length === 0 ? <div className="text-center py-12 text-slate-500 italic bg-slate-900/30 rounded-lg border border-slate-800 border-dashed">The wind howls... there are no stories here yet.</div> :
            threads.map(thread => {
              ...
              const isLocked = thread.isLocked === true;
              return (
                  ...
                  <div className="flex justify-between items-start pl-2">
                    <div>
                      <h3 className={`text-lg font-bold transition-colors flex items-center gap-2 ${isUnread ? 'text-cyan-100' : 'text-slate-200 group-hover:text-amber-400'}`}>
                        {thread.title}
                        {isLocked && (
                          <span className="flex items-center gap-1 text-[10px] bg-amber-900/30 text-amber-500 border border-amber-700/50 px-1.5 rounded uppercase tracking-wider font-bold" title="Sacred Text">
                            <BookLock className="w-3 h-3" /> Sacred
                          </span>
                        )}
